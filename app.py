...
with st.form("bill_form"):
    ...
    address = st.text_input("住所（都道府県〜部屋番号まで）", placeholder="例：東京都渋谷区神宮前1-2-3", value="")
    st.markdown("---")
    st.subheader("② 請求月の選択")
    today = date.today()
    col3, col4 = st.columns(2)
    with col3:
        year = st.selectbox("年", options=list(range(today.year - 1, today.year + 2)), index=1)
    with col4:
        month = st.selectbox("月", options=list(range(1, 13)), index=today.month - 1, format_func=lambda m: f"{m}月")
    st.markdown("---")
    st.subheader("③ 使用量の設定")
    auto_kwh = st.checkbox("季節に合わせて自動計算する（推奨）", value=True)
    manual_kwh = None
    if not auto_kwh:
        manual_kwh = st.slider("使用量（kWh）", min_value=50, max_value=600, value=200, step=5)
    st.markdown("---")
    st.subheader("④ 発行日の指定")
    issue_date_input = st.date_input("発行日", value=date(today.year, today.month, 25))
    submitted = st.form_submit_button("📄　明細書を生成する", use_container_width=True)

if submitted:
    errors = []
    if not name.strip(): errors.append("氏名を入力してください。")
    if not postal.strip(): errors.append("郵便番号を入力してください。")
    if not address.strip(): errors.append("住所を入力してください。")
    if errors:
    ...
    with st.spinner("明細書を生成中…"):
        ...
        sp = "-".join([f"{random.randint(0,99):02d}"] + [f"{random.randint(0,9999):04d}" for _ in range(5)])
        data = dict(
            name=name.strip(), postal=postal.strip(), address=address.strip(),
            contract=contract, invoice=invoice, issue_date=issue_date,
...
| **最終請求金額** | **{bill['final_total']:,}円** |
    ...
    st.download_button(label="⬇️　PDFをダウンロード", data=pdf_bytes, file_name=filename, mime="application/pdf", use_container_width=True)
